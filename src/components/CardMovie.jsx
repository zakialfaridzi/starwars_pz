...
import { MoviesContext } from "./MoviesSection";

export default function CardMovie({ characters }) {
  const mov = useContext(MoviesContext);
  ...
  return mov.map((movie) => {
    const { title, episode_id, director, release_date, opening_crawl } = movie;
    return (
      <Box
        maxW={"full"}
        w={"full"}
